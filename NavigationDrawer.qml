...
Rectangle {
    ...
    // Conversion of density independent pixels into physical pixels of the device
    readonly property int dpi: Screen.pixelDensity * 25.4
    function dp(x){ return (dpi < 120) ? x : x*(dpi/160); }

    // Navigation Drawer Properties
    property bool open: false           // State of Navigation Drawer - Open/closed
    property int position: Qt.LeftEdge  // Navigation Drawer position - Left/right

    // Functions for opening and closing the Navigation Drawer
    function show() { open = true; }
    function hide() { open = false; }
    function toggle() { open = open ? false : true; }

    // Internal properties of Navigation Drawer
    ...
    MouseArea {
        ...
        drag.onActiveChanged: if (open) holdAnimation.stop()
